...
static void insert_char(char c) {
    if (osk_cursor_pos < sizeof(osk_buf) - 1) {
        memmove(&osk_buf[osk_cursor_pos + 1], &osk_buf[osk_cursor_pos], sizeof(osk_buf) - osk_cursor_pos - 1);
    ...
    if (input_pad() & BUTTON_Z) {
        switch (nav) {
            ...
            case MENU_NAVIGATE_LEFT: {
                if (osk_cursor_pos > 0) {
                    memmove(&osk_buf[osk_cursor_pos - 1], &osk_buf[osk_cursor_pos], sizeof(osk_buf) - osk_cursor_pos);
                    --osk_cursor_pos;
                }
                break;
            }
...
static s32 draw_tooltip_proc(struct menu_item *item, struct menu_draw_params *draw_params) {
    ...
    draw_params->y += ch;
    draw_shortcut(item, draw_params, bind_make(2, BUTTON_Z, BUTTON_D_LEFT), "backspace");
    draw_params->y += ch;
    draw_shortcut(item, draw_params, bind_make(2, BUTTON_Z, BUTTON_D_DOWN), "space");
    draw_params->y += ch;
    draw_shortcut(item, draw_params, bind_make(2, BUTTON_Z, BUTTON_D_RIGHT), "delete");

    if (item->owner->selector == osk_text) {
        draw_params->y += ch;
        draw_shortcut(item, draw_params, bind_make(1, BUTTON_D_LEFT), "cursor left");
        draw_params->y += ch;
        draw_shortcut(item, draw_params, bind_make(1, BUTTON_D_RIGHT), "cursor right");
        gfx_mode_set(GFX_MODE_COLOR, GPACK_RGB24A8(draw_params->color, draw_params->alpha));
        draw_params->y += ch;
        gfx_printf(draw_params->font, draw_params->x, draw_params->y, "press to confirm");
    }
