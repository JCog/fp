...
static void bossWarpProc(struct MenuItem *item, void *data) {
    ...
    pm_clear_printers();
    // CONTEXT_BATTLE
    // TODO: verify that removing the game mode checks is fine here
    if (pm_gGameStatus.context == 1 && pm_CurGameMode != 10 && pm_CurGameMode != 11) {
...
void bossesUpdateWarps() {
    if (leavingBattle && pm_gGameStatus.context != 1) { // CONTEXT_BATTLE
        leavingBattle = FALSE;
        bossWarp();
